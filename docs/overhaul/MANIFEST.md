...
| `bsm3/core/boundary_surface_movement/wall_surface.pkl` | S | Curated, tracked legacy mixed-N-gon regression asset. The curated test uses the explicitly named trusted-pickle API; the E175 pipeline still has an internal legacy pickle branch. Conversion to a non-executable format and removal of that branch remain follow-up work. |
...
| `bsm3/core/projections/refitted_fun_set.pkl` | S, D, P, M, L | Legacy default used by warm-start projection utilities when that path is selected; untracked and executable pickle, so M3 must either prove it unused or replace it. |
