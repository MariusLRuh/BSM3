...
| M1.7 | Acceptance run. Must exercise a **STEP-to-VortexAD path** if practical, alongside the surface-motion path — the VortexAD root is the one the Turn-1 trace missed entirely, so it is the least protected by existing tests. **Turn-27 carry-ins:** (a) consider a `TypedDict` for `GraphDistanceWeighting.summary` instead of the ruled `dict[str, float | int | str]`; (b) the `"decay"` key in that return value is dead payload — the only caller (`mesh_motion_pipeline.py:941`) never reads it — so removing it would allow `dict[str, float | int]`. Both are public-return changes, deliberately out of scope for the M1.6 docstring slices. | Claude implements/runs; Codex plans/reviews | **READY FOR CODEX ACCEPTANCE, not self-accepted** (Turn 58: `338fde8`, docs commit). All five API findings are closed: typed `GraphDistanceSummary` with the dead `"decay"` entry removed and the type exported; `DerivativeComparison.best` returning `tuple[float | None, float]`; `E175DAFoamResult.mesh_motion` optional; both formerly dead `FlowConfig` fields wired into `primalBC` (`nuTilda0` farfield BC and `useWallFunction`) with a single `--wall-functions` BooleanOptionalAction; and `plot_components(colors=None)` preserving component colors. **R4 reproduced bit-for-bit** against a pristine `TURN58_BASE` clone: identical sha256, max coordinate difference exactly 0.0, identical inversion and degenerate ID arrays at all three stages, identical fold/cell counts and all 13 quality scalars. Flagship E175 18/18 including the five named integration cases (external-coefficient relative error 4.196e-14 against a 1e-5 threshold, max displacement 0.3502, quad panel 114 inversions preserved with none added, zero folds everywhere); curated mixed-N-gon 3 passed; derivative/N-gon exactly 6 passed. VortexAD remains **impractical with recorded evidence**, not waived. | Generalized driver reproduces current R4 output within tolerance; derivative gate passes; quad and mixed-N-gon examples clean; a STEP-to-VortexAD path runs end to end or is recorded as impractical with the reason. |
