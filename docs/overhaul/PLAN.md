...
| M1.7 | Acceptance run. Must exercise a **STEP-to-VortexAD path** if practical, alongside the surface-motion path — the VortexAD root is the one the Turn-1 trace missed entirely, so it is the least protected by existing tests. **Turn-27 carry-ins:** (a) consider a `TypedDict` for `GraphDistanceWeighting.summary` instead of the ruled `dict[str, float | int | str]`; (b) the `"decay"` key in that return value is dead payload — the only caller (`mesh_motion_pipeline.py:941`) never reads it — so removing it would allow `dict[str, float | int]`. Both are public-return changes, deliberately out of scope for the M1.6 docstring slices. | Claude implements/runs; Codex plans/reviews | **CORRECTION REQUIRED — not accepted** (Turn 59 review of Turn 58). The five Turn-58 API corrections and their numerical gates are sound, but final acceptance exposed an untested live-chain regression from M1.1: the default rank-0 STEP→surface→volume→DAFoam path references an undefined parameterization factory, `MeshMotionVolumeBackend`'s caller and constructor disagree on `input_files` versus `model_files`, and its `_build_model` still calls `run_mesh_motion` with the removed `model_files`/`geometry_parameterization` keywords. The derivative ladder calls the same removed factory. Four retained-production `F821` errors also escaped CI because the critical Ruff gate still covers only the M0 subset. Turn 59 issues one narrow correction with mock-backed execution tests and expands critical static coverage over the retained production manifest. | Generalized driver reproduces current R4 output within tolerance; derivative gate passes; quad and mixed-N-gon examples clean; a STEP-to-VortexAD path runs end to end or is recorded as impractical with the reason; the retained rank-0 STEP→surface→volume backend constructs through the current public API under an executable no-DAFoam regression. |
