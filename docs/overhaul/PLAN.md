...
| M4.1a | Lock the GAMMA public identity and plan a clean-break migration before the first public documentation release. Distinguish the display name, repository slug, Python distribution, import namespace, and RTD slug rather than assuming they must all be bare `gamma`. | Claude plans; user rules; Codex implements | **promoted ahead of M4.2 by user steering after Turn 68** | The expansion and every technical identifier are explicit; bare `gamma` is rejected as an unavailable/overloaded distribution name; version policy is reconciled with the current `0.1.4`; the dirty tree has a concrete preservation strategy; the implementation prompt has a literal allowlist and clean-clone acceptance. |
| M4.1b | Publish an initial versioned GAMMA alpha on Read the Docs after the identity migration. | Claude plans; Codex prepares; user authorizes external publication | queued after M4.1a | Strict HTML documentation builds under the final public identity from pinned docs-only dependencies; the RTD project and tag policy are explicit; no repository rename, tag, push, PyPI reservation, or RTD mutation occurs without user authorization. |
