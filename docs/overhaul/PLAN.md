...
| M1.6 | Numpydoc docstrings across the public surface of the live core. **M1.6 owns the documentation debt that M0.4 staged out of CI**: repo-wide critical ruff currently reports 398 errors in legacy/experimental files, and the measured numpydoc baseline is 0 sectioned public definitions. Widening the CI lint gate from the M0 file list to the retained manifest is part of this task. | Claude implements; Codex plans/reviews | slices 1-2 **COMPLETE** (slice 2 accepted Turn 47 after independent AST, contract, and 82+6 regression checks); slice 3 **READY FOR CODEX REVIEW, not self-accepted** (`cdcba68`): 17/17 module docstrings, 167/167 public definitions, 132 callables at 293/293 documented parameters (0 missing / 0 extra), 22 dataclasses at 148/148 documented fields (0 missing / 0 extra), 17/17 stripped-AST identity in working tree and committed blobs, 89+6 regression tests green, and the exact 17-path `ruff --select D` command passing. **Blocking finding:** the two pre-existing CI documentation steps for the surface-motion core and for projection/preprocessing currently fail `ruff --select D` with 123 and 48 errors, so adding this step does not by itself make CI green | ruff pydocstyle clean over the retained tracked manifest; coverage >=90% of public defs; CI lint scope widened from the M0 file list. |
