...
| M1.6 | Numpydoc docstrings across the public surface of the live core. **M1.6 owns the documentation debt that M0.4 staged out of CI**: repo-wide critical ruff currently reports 398 errors in legacy/experimental files, and the measured numpydoc baseline is 0 sectioned public definitions. Widening the CI lint gate from the M0 file list to the retained manifest is part of this task. | Claude implements; Codex plans/reviews | slices 1-2 **COMPLETE**; Turn 54 closed both stale CI doc-lint gates and preserved all structural/numerical evidence, independently reproduced at Turn 55: five Ruff commands green, exact coverage audits, 38/38 stripped-AST identity, and 82+3 deselected / 89 / 6 tests. Turn 56 completed that final five-file prose correction (`b999476`): regular-package terminology, root/non-inline optional result behavior, the two omitted RBF value constraints, the parser-default boundary and the two missing exception contracts, and raw mean-ratio sign versus relative inversion. All seven were verified in the implementation first. Five Ruff commands green, coverage audit unchanged, 5/5 stripped-AST identity against `139f35c` in tree and committed blobs, 82+3 deselected / 89 / 6 tests. The Turn-54 entry and the M1.7 carry-in wording now carry inline corrections so they no longer repeat the rejected claims. **M1.6 is ready for Codex acceptance, not self-accepted**; M1.8 remains next after acceptance, and four public/API carry-ins stand for M1.7. | ruff pydocstyle clean over the retained tracked manifest; coverage >=90% of public defs; CI lint scope widened from the M0 file list. |
