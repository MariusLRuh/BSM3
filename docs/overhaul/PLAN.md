...
| M1.6 | Numpydoc docstrings across the public surface of the live core. **M1.6 owns the documentation debt that M0.4 staged out of CI**: repo-wide critical ruff currently reports 398 errors in legacy/experimental files, and the measured numpydoc baseline is 0 sectioned public definitions. Widening the CI lint gate from the M0 file list to the retained manifest is part of this task. | Claude implements; Codex plans/reviews | slices 1-2 **COMPLETE**; slice 3 structural work independently verified at Turn 53 (17/17 modules, 167/167 definitions, exact 293-parameter and 148-field audits, 17/17 AST identity, 89+6 tests, and its Ruff step green) but **M1.6 remains open**. Turn 54 completed that closure (`139f35c`): **all four** `actions.yml` documentation steps and the default critical-static-checks command now pass under `ruff 0.9.10`, the surface-core and projection/preprocessing failures going 123 and 48 to zero; all eleven named slice-3 semantic inaccuracies are corrected against the bodies; and the historical M1.9 Ruff wording is qualified as the repository default selection. 38 changed files, stripped-AST identical to `81736be` in both the working tree and the committed blobs; 82+3 deselected, 89, and 6 tests green. **M1.6 is ready for Codex acceptance, not self-accepted.** Four public/API carry-ins are recorded for M1.7. | ruff pydocstyle clean over the retained tracked manifest; coverage >=90% of public defs; CI lint scope widened from the M0 file list. |
