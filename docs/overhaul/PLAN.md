...
| M1.9 | **New (2026-09-24).** Migrate off the forked geometry stack onto official `LSDOlab/CSDL_alpha` main `73a9efd` and official `LSDOlab/lsdo_function_spaces` main `307ad3a`, on Python 3.12 / NumPy 2.0.2 / SciPy 1.13.1 / JAX 0.4.38. Removes BSM3's dependence on the forked `compute_basis_matrix_numpy_factory_patched` shim and ports the useful local STEP-importer fix upstream. | Claude | **TECHNICALLY ACCEPTED BY CODEX; READY FOR FINAL ACCEPTANCE, not self-accepted** (BSM3 `bf2afee`, LFS `b6e7b4e`, docs correction follows): 0 tracked production Python imports of the patched factory and 0 tracked references outside `docs/overhaul/` (descriptive mentions inside the collaboration documents are expected and retained), factory byte-identity proven by sha256, 192/192 BSM3 tests pass with the compatibility alias removed, E175 coordinates reproduce at max abs diff 0.0. | No tracked import of the patched factory; 192 tests green on official mains; E175 within rtol=atol=1e-14; CI pinned to both official commits with LFS installed `--no-deps`. |
...
| M1.7 | Acceptance run. Must exercise a **STEP-to-VortexAD path** if practical, alongside the surface-motion path — the VortexAD root is the one the Turn-1 trace missed entirely, so it is the least protected by existing tests. **Turn-27 carry-ins:** (a) consider a `TypedDict` for `GraphDistanceWeighting.summary` instead of the ruled `dict[str, float | int | str]`; (b) the `"decay"` key in that return value is dead payload — the only caller (`mesh_motion_pipeline.py:941`) never reads it — so removing it would allow `dict[str, float | int]`. Both are public-return changes, deliberately out of scope for the M1.6 docstring slices. | Claude implements/runs; Codex plans/reviews | **READY FOR CODEX ACCEPTANCE, not self-accepted** (Turn 59: `e47d07f`, docs commit). The rank-0 STEP→surface→volume chain now constructs through the current API. `create_geometry_parameterization_from_variables` exists and is shared with `create_geometry_model` through one private population helper, so the two entry points cannot drift; `MeshMotionVolumeBackend` takes `input_files` and calls `run_mesh_motion(input_files=…, geometry=…)`, with both retained call sites updated and the removed keyword now raising `TypeError`; and the three communicator annotations resolve through a runtime-checkable structural protocol with no eager `mpi4py` import and no `noqa`. Six regressions guard the boundary, each verified to fail at `b60f0c0`, including a mock-backed backend execution test and a rank-0 construction test that mocks only expensive I/O and the downstream operation. The **Critical static checks** step now runs default Ruff over all 50 retained production paths plus the three M0 test files, closing the hole that let the four `F821` errors through; `ruff.toml` is unchanged and no per-file ignore was added. Five modules 156 passed (150 + exactly the 6 new tests), derivative/N-gon exactly 6, all five literal workflow Ruff commands green, reconstructed production manifest clean. | Generalized driver reproduces current R4 output within tolerance; derivative gate passes; quad and mixed-N-gon examples clean; a STEP-to-VortexAD path runs end to end or is recorded as impractical with the reason; the retained rank-0 STEP→surface→volume backend constructs through the current public API under an executable no-DAFoam regression. |
