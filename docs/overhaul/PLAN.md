...
| M4.1a | Lock the GAMMA public identity and plan a clean-break migration before the first public documentation release. Distinguish the display name, repository slug, Python distribution, import namespace, and RTD slug rather than assuming they must all be bare `gamma`. | Claude plans; user rules; Codex implements | **RULED — Turn 70; prompt issued.** Display `GAMMA`, expansion *Geometry Adaptation for Multidisciplinary Modeling and Analysis*, repository `GAMMA-MDO`, distribution `gamma-mdo` (PyPI 404 = unoccupied, **not reserved**), RTD slug `gamma-mdo` (RTD API v3: *No Project matches the given query*). **The import namespace deliberately stays `bsm3`.** 304 of 393 untracked entries and 6 of 8 modified tracked files live under `bsm3/`; 57 untracked user scripts and 5 modified tracked files import `bsm3`, and no agent may edit them. Version ruled **v0.2.0a1**, forward-moving from the `0.1.4` that source installs already report; `bsm3` was never published to PyPI and neither the local repo nor `origin` has any tag, so no public release exists to regress. | The expansion and every technical identifier are explicit; bare `gamma` is rejected as an unavailable/overloaded distribution name; version policy is reconciled with the current `0.1.4`; the dirty tree has a concrete preservation strategy; the implementation prompt has a literal allowlist and clean-clone acceptance. |
| M4.1b | Publish an initial versioned GAMMA alpha on Read the Docs after the identity migration. | Claude plans; Codex prepares; user authorizes external publication | **PLANNED — Turn 70; prompt issued.** HTML only: `.readthedocs.yaml` currently declares `formats: [pdf, htmlzip]`, neither of which has ever been built, while `fail_on_warning: true` would apply to an unverified LaTeX stage. The build stays hermetic — `docs/conf.py` reads `__version__` by regex without importing the package, so no CSDL_alpha, LFS, JAX, gmsh, DAFoam, MPI or VortexAD is needed. Repository rename, RTD project creation and webhook, tag creation and push, and any PyPI reservation are enumerated as **user-authorized external actions** that no agent performs. | Strict HTML documentation builds under the final public identity from pinned docs-only dependencies; the RTD project and tag policy are explicit; no repository rename, tag, push, PyPI reservation, or RTD mutation occurs without user authorization. |
| M4.2 | Add a tracked VortexAD adapter and fuel-burn optimization example against pinned external revisions, using `mm.run` rather than resurrecting removed APIs. | Codex implements; Claude reviews | **planned in Turn 68 and queued after M4.1a/M4.1b.** The complete implementation prompt is preserved at `38cd0bf:docs/overhaul/CODEX_NEXT.md`. | A supported panel solve and optimization run from tracked code; analytic derivatives are checked through the geometry-to-aerodynamics objective; DAFoam remains optional. |
