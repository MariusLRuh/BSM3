...
| ID | Task | Owner | Status | Acceptance |
|----|------|-------|--------|------------|
| M2.1 | Replace the template site with a buildable BSM3 Sphinx/Read-the-Docs site: validated installation, five-stage E175 quickstart, external-parameterization contract, compact public API reference, optional-integration boundaries, troubleshooting/assets, and concise README links. | Claude implements; Codex plans/reviews | **READY FOR CODEX REVIEW, not self-accepted** (Turn 61: `4ca0435`, docs commit). The template scaffold is replaced, not layered over: nine obsolete pages deleted and the site reduced to a landing page plus Installation, E175 example, External parameterization, API, Background, and Integrations. The build is hermetic — hand-written Markdown, version read from source by regex rather than importing `bsm3`, no autoapi/notebook/collection machinery — so it needs none of the geometry stack and never executes the example. Sphinx 9.1.0 on Python 3.12.14 in a disposable `/tmp` venv builds **warning-clean under `-W --keep-going`**, as does a fresh clone of the implementation commit whose status stays empty. `tests/test_documentation.py` adds 10 structural guards; fast E175 example tests 13 passed / 5 deselected; default Ruff on the new files and all five literal workflow Ruff commands green. | Fresh Python 3.12 docs environment builds HTML with warnings as errors; CI carries the same gate; no template prose or broken toctree target remains; docs do not require DAFoam, VortexAD, MPI, or local untracked assets to build. |
