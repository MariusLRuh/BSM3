...
| M1.6 | Numpydoc docstrings across the public surface of the live core. **M1.6 owns the documentation debt that M0.4 staged out of CI**: repo-wide critical ruff currently reports 398 errors in legacy/experimental files, and the measured numpydoc baseline is 0 sectioned public definitions. Widening the CI lint gate from the M0 file list to the retained manifest is part of this task. | Codex | slice 1 COMPLETE; **slice 2 SECOND CORRECTION READY FOR CODEX REVIEW** (Turn 44: 6/6 stripped-AST identity, 80/80 public defs and 15/15 module docstrings, 36/36 public callables at 0 missing / 0 extra documented parameters, 0 normalized rejection hits, 4 VJP mappings and `unsort` return containers audited against their `return` statements); slice 3 OPEN | ruff pydocstyle clean over the retained manifest; coverage >=90% of public defs; CI lint scope widened from the M0 file list. |
