...
| M4.2 | Add a tracked VortexAD adapter and fuel-burn optimization example against pinned external revisions, using `mm.run` rather than resurrecting removed APIs. | Codex implements; Claude reviews | **IMPLEMENTED — ready for Claude review, not accepted.** Commit `027a890` adds the optional adapter, VortexAD-free Breguet model, tracked five-stage example, documentation, and 12 passing tests plus one explicitly skipped real-VortexAD integration check. The fake-solver derivative test carries a mesh-shape design variable through panel outputs and fuel burn before comparing the analytic VJP with centered finite differences. Official VortexAD main is pinned at `8c5bc86fda5fa1e359fecde24c6c6e8527c773b5`. No real VortexAD, DAFoam, OpenFOAM, or MPI solve ran. | A supported panel solve and optimization run from tracked code; analytic derivatives are checked through the geometry-to-aerodynamics objective; DAFoam remains optional. |
