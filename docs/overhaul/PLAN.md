...
| M1.8 | **New (Turn 5).** Retire the internal legacy polygon-pickle branch in the E175 pipeline. Turn 4 made the *public* importer safe by removing `.pkl` from suffix dispatch, but the pipeline retains an internal trusted-pickle path, and `bsm3/core/projections/refitted_fun_set.pkl` is an untracked executable pickle used as a warm-start default. Convert `wall_surface.pkl` to `.npz` per `ASSETS.md` and delete the branch. | Claude implements; Codex plans/reviews | **in progress — Turn 57 prompt issued.** The replacement schema is fixed as non-object `vertices`, flattened `connectivity`, and `offsets`, loaded with `allow_pickle=False`; exact legacy hashes and interleaved face order are acceptance evidence. The retained warm-start API must no longer supply an untracked default path, while explicitly named trusted-pickle entry points may remain. | No pickle load remains reachable from any retained root except through an explicitly named trusted API; `wall_surface.pkl` replaced by a non-executable container with identical coordinates and connectivity. |
