...
| M1.8 | **New (Turn 5).** Retire the internal legacy polygon-pickle branch in the E175 pipeline. Turn 4 made the *public* importer safe by removing `.pkl` from suffix dispatch, but the pipeline retains an internal trusted-pickle path, and `bsm3/core/projections/refitted_fun_set.pkl` is an untracked executable pickle used as a warm-start default. Convert `wall_surface.pkl` to `.npz` per `ASSETS.md` and delete the branch. | Claude implements; Codex plans/reviews | **READY FOR CODEX ACCEPTANCE, not self-accepted** (Turn 57: `2449441`, `4a39fd8`, docs commit). `wall_surface.npz` replaces the deleted pickle with non-object `vertices`, flattened `connectivity`, and `offsets` loaded with `allow_pickle=False`; all three decoded-array hashes, the width counts, the node-ID range, and the 14,720 adjacent width transitions that evidence original face order reproduce exactly. `.npz` is generic suffix-dispatched; pickle is not. The pipeline's `_load_polygon_surface_pickle` and `_cfd_mesh_from_pickle` are gone, `polygon_connectivity` now derives from the imported mesh's stored order, and `_cfd_surface_cells` is retained for the width-grouped quality order. `DEFAULT_FUN_SET_PATH` is deleted and the two loaders are consolidated into `load_function_set_from_trusted_pickle(pickle_path)` with a mandatory path. The only reachable pickle loads are inside that function and `import_trusted_polygon_pickle`. Tests 97+3 deselected / 102 / 6 / 3 integration, all five Ruff commands green. | No pickle load remains reachable from any retained root except through an explicitly named trusted API; `wall_surface.pkl` replaced by a non-executable container with identical coordinates and connectivity. |
