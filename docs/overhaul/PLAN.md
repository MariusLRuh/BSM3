...
| M1.7 | Acceptance run. Must exercise a **STEP-to-VortexAD path** if practical, alongside the surface-motion path — the VortexAD root is the one the Turn-1 trace missed entirely, so it is the least protected by existing tests. **Turn-27 carry-ins:** (a) consider a `TypedDict` for `GraphDistanceWeighting.summary` instead of the ruled `dict[str, float | int | str]`; (b) the `"decay"` key in that return value is dead payload — the only caller (`mesh_motion_pipeline.py:941`) never reads it — so removing it would allow `dict[str, float | int]`. Both are public-return changes, deliberately out of scope for the M1.6 docstring slices. | Claude implements/runs; Codex plans/reviews | **in progress — Turn 58 prompt issued.** Five accumulated public/API corrections are bounded before the final numerical acceptance run. R4 is available locally and must reproduce a pre-edit surface baseline. The tracked large tri, quad, external-coefficient derivative, whole-free-component, polygon6, and mixed-N-gon gates all run. VortexAD is currently recorded as impractical: absent from the validated environment; the sibling checkout is dirty and fails import on missing `vedo`; the untracked panel driver also targets removed APIs. No external installation or adoption is authorized in this turn. | Generalized driver reproduces current R4 output within tolerance; derivative gate passes; quad and mixed-N-gon examples clean; a STEP-to-VortexAD path runs end to end or is recorded as impractical with the reason. |
