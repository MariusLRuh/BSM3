...
| M1.9 | **New (2026-09-24).** Migrate off the forked geometry stack onto official `LSDOlab/CSDL_alpha` main `73a9efd` and official `LSDOlab/lsdo_function_spaces` main `307ad3a`, on Python 3.12 / NumPy 2.0.2 / SciPy 1.13.1 / JAX 0.4.38. Removes BSM3's dependence on the forked `compute_basis_matrix_numpy_factory_patched` shim and ports the useful local STEP-importer fix upstream. | Claude | **COMPLETE — accepted Turn 60.** The Turn-50 technical acceptance is final: zero tracked production Python imports of the patched factory and zero tracked references outside `docs/overhaul/`, factory byte identity proven, 192 BSM3 tests passed with the alias removed, and the E175 coordinates reproduced at max absolute difference 0.0. CI remains pinned to the official CSDL and LFS commits; the separate LFS importer fix is locally committed but must not become a BSM3 dependency until it lands upstream. | No tracked import of the patched factory; 192 tests green on official mains; E175 within rtol=atol=1e-14; CI pinned to both official commits with LFS installed `--no-deps`. Met. |
| M1.6 | Numpydoc docstrings across the public surface of the live core. **M1.6 owns the documentation debt that M0.4 staged out of CI**: repo-wide critical ruff currently reports 398 errors in legacy/experimental files, and the measured numpydoc baseline is 0 sectioned public definitions. Widening the CI lint gate from the M0 file list to the retained manifest is part of this task. | Claude implements; Codex plans/reviews | **COMPLETE — accepted Turn 57.** Turn 54 closed both stale CI doc-lint gates; Turn 56 corrected the final seven semantic inaccuracies. Codex independently reproduced the exact eight-path range, clean whitespace, five Ruff commands, 5/5 tree and blob stripped-AST identity, the complete coverage audit, and all three regression suites at 82+3 deselected / 89 / 6. Four executable/API carry-ins remain explicitly assigned to M1.7. | ruff pydocstyle clean over the retained tracked manifest; coverage >=90% of public defs; CI lint scope widened from the M0 file list. Met. |
| M1.8 | **New (Turn 5).** Retire the internal legacy polygon-pickle branch in the E175 pipeline. Turn 4 made the *public* importer safe by removing `.pkl` from suffix dispatch, but the pipeline retains an internal trusted-pickle path, and `bsm3/core/projections/refitted_fun_set.pkl` is an untracked executable pickle used as a warm-start default. Convert `wall_surface.pkl` to `.npz` per `ASSETS.md` and delete the branch. | Claude implements; Codex plans/reviews | **COMPLETE — accepted Turn 58** (`2449441`, `4a39fd8`, `e94e669`). Codex independently reproduced the 13-path range, safe decoded hashes/order, direct consolidated-loader call, 54 / 97+3 deselected / 102 / 6 / 3-integration tests, and all five Ruff commands. The retained pipeline and curated assets have no implicit pickle path. The non-retained dirty research script remains broken by the deliberate asset removal and requires both a path update and replacement of its private pickle loader; it is deferred to its owner/M3 rather than touched through this clean boundary. | No pickle load remains reachable from any retained root except through an explicitly named trusted API; `wall_surface.pkl` replaced by a non-executable container with identical coordinates and connectivity. Met. |
...
| M1.7 | Acceptance run. Must exercise a **STEP-to-VortexAD path** if practical, alongside the surface-motion path — the VortexAD root is the one the Turn-1 trace missed entirely, so it is the least protected by existing tests. **Turn-27 carry-ins:** (a) consider a `TypedDict` for `GraphDistanceWeighting.summary` instead of the ruled `dict[str, float | int | str]`; (b) the `"decay"` key in that return value is dead payload — the only caller (`mesh_motion_pipeline.py:941`) never reads it — so removing it would allow `dict[str, float | int]`. Both are public-return changes, deliberately out of scope for the M1.6 docstring slices. | Claude implements/runs; Codex plans/reviews | **COMPLETE — accepted Turn 60** (`338fde8`, `e47d07f`, supporting documentation commits). Codex independently inspected the repaired factory/backend chain and reproduced 156 focused tests, exactly 6 derivative/N-gon tests, default Ruff over the complete 50-path retained production manifest, and the union of all documentation-lint paths. The Turn-58 numerical evidence remains binding: bit-identical R4 output, 18 E175 example tests, external-coefficient derivative relative error 4.196e-14, and clean triangle/quad/N-gon gates. VortexAD is an evidence-backed deferral to a tracked adapter against a pinned clean revision, not a claimed integration. | Generalized driver reproduces current R4 output within tolerance; derivative gate passes; quad and mixed-N-gon examples clean; VortexAD impracticality and the required adapter are recorded; the retained rank-0 STEP→surface→volume backend constructs through the current public API under executable no-DAFoam regressions. Met. |

**M1 is complete as of Turn 60.** The supported boundary is the generalized
surface API and its STEP→surface→volume/aerodynamic adapter contract. A real
DAFoam solve was never required for M1; a tracked VortexAD adapter remains a
later optional-integration task.
...
### Turn-47 Codex review: **M1.6 slice 2 ACCEPTED**; slice 3 planned

Slice 2 is accepted. Codex independently reproduced the exact three-source
path set, **3/3** stripped-AST identity against `4e83fa6`, a clean scoped diff,
**82 passed / 3 deselected**, and **6 passed** in the derivative/N-gon guards.
The corrected convergence evidence, retry ranking followed by compatibility,
and eager sampling costs match the actual implementations. The normalized
phrase check remains only a regression backstop; acceptance rests on reading
the bodies and tests.

Slice 3 covers the remaining **tracked** release surface: 17 Python modules,
8,763 LOC, 167 public definitions, 132 public callables with 293 parameters,
and 22 dataclasses with 148 locally declared fields. The untracked
`e175_panel_opt.py` is deliberately excluded: Turn 5 deferred its adoption
until M1.7 supplies a VortexAD end-to-end test, and a documentation slice must
not silently reverse that ruling. The prompt requires full signature and
dataclass-field contracts, stripped-AST identity, the fake-backend/MPI/volume
test baseline of 89 passes, and the derivative/N-gon baseline of 6 passes.

---

### Turn-45 Codex review: M1.6 slice 2 final semantic correction

Turn 44 closes the structural documentation gap. Codex independently
reproduced **6/6** stripped-AST identity, the embedded **36 callables / 124
parameters / 0 missing / 0 extra** audit, **80/80** definition coverage,
**82 passed / 3 deselected**, and **6 passed** in the derivative/M1.4 guards.
The return mappings, `unsort` container, dataclass fields, buffer callbacks,
and seven named semantic errors are corrected.

Slice 2 is held for three narrow accuracy fixes introduced or retained in the
expanded prose. `SurfaceProjectionResult.uv` says `converged` is the only
convergence evidence, while `FunctionSetProjectionModel.project` still says
the residual is the only evidence; both objects expose the Boolean plus
residual/iteration/step diagnostics. The retry prose twice says a farther
candidate can replace a converged clamped candidate, but `_is_candidate_better`
requires the retry to be closer when both candidates are converged. A farther
retry is possible only when it improves the convergence rank (selected
non-converged, retry converged), and the compatibility distance cap then
applies. “Higher setup cost” is also replaced with eager construction cost.

**Standing gate ruling:** punctuation-insensitive normalized phrase matching
is retained as a regression backstop for known rejected claims, but it is not
an accuracy proof and will not be expanded indefinitely. Structural contracts
belong in AST/signature/return-container audits; semantic accuracy still
requires reading the implementation and tests. Turn 46 is restricted to the
three affected source modules and collaboration docs.

---

### Turn-43 Codex review: M1.6 slice 2 second correction required

The Turn-42 correction again preserves executable behavior: Codex reproduced
**6/6** stripped-AST identity, **82 passed / 3 deselected**, **6 passed** in the
derivative/M1.4 guards, and a clean scoped diff. The path and history controls
also pass.

Slice 2 remains open. The line-oriented rejection grep was defeated by the
very line wrapping Turn 42 reported: `reject or` and `down-weight` remain on
adjacent lines in `build_degenerate_edge_map`. The class docstring for
`FunctionSetEvaluationModel` remains untouched and still says coordinates are
fixed at construction and represented by a cached basis row. Four CSDL VJP
`evaluate` methods document tuples although they return keyed dictionaries;
`unsort` documents a tuple and `*arrays` although it returns a list and accepts
`*arrays_sorted`. A stale `boundary_clamped` field comment repeats the rejected
"converged only because" interpretation.

The signature audit also shows that docstring presence/style is still too weak
as an acceptance measure: across 36 public callables in the six projection
modules, **79 signature parameters are absent from `Parameters` sections**.
This includes nearly the entire 30-argument public candidate-projection API.
Turn 44 replaces the line grep with whitespace-normalized source inspection,
requires zero missing/extra documented parameters, and pins exact return
containers.

---

### Turn-41 Codex review: M1.6 slice 2 correction required

The source/CI commit is contained and executable behavior is unchanged:
Codex independently reproduced **15/15** stripped-AST identity, **80/80**
public-docstring presence, a valid nine-step workflow, **82 passed / 3
deselected** in the focused suite, and **6 passed** in the derivative/M1.4
guards. The accidentally staged research artifacts are absent from both final
commits and remain untracked; the recovery is accepted and needs no repository
repair.

Slice 2 is not accepted because presence is not accuracy. The new prose calls
parametric coordinates fixed/cached at setup even though they are supplied per
evaluation and receive a VJP; describes eager Newton and warm-start work as
setup-only even though custom-operation `compute` invokes it; gives incorrect
VJP parameter names and return ordering; says degenerate edges are excluded or
down-weighted when they are converted to fixed-point candidates; and says the
residual is the only convergence evidence despite the stored `converged` flag.
The coefficient order also defaults to ascending patch ID but follows explicit
`patch_indices` when supplied.

In addition, `warm_start_projections.py` still has a leading-blank module
docstring and seven legacy public docstrings with leading blanks and prose such
as `Returns:` instead of NumPy sections. The newly added Ruff step is therefore
not treated as satisfied merely because Ruff is unavailable locally. Turn 42
corrects and audits the six projection modules without changing executable AST
or reopening the accepted CI diff.
...
| `wall_surface.npz` | `tests/test_curated_assets.py`, `@pytest.mark.integration` plus two fast decoded-array checks | Loads via the generic safe importer and assembles with the expected mode count. **No solve, no derivative** — the assembly must not enter the fast gate. Pickle retirement completed in M1.8 (Turn 57). |
...
| ID | Task | Owner | Status | Acceptance |
|----|------|-------|--------|------------|
| M2.1 | Replace the template site with a buildable BSM3 Sphinx/Read-the-Docs site: validated installation, five-stage E175 quickstart, external-parameterization contract, compact public API reference, optional-integration boundaries, troubleshooting/assets, and concise README links. | Claude implemented initial site; Codex implemented correction; Claude reviews | **ACCEPTED — Turn 64** (`4ca0435`, `ab79087`). Claude independently reproduced every claim. The one-path widening by `requirements-ci.txt` is **accepted as a necessary packaging fix**: `lsdo_b_splines_cython@9444ea8` hard-pins `numpy==1.26.4` in both its `pyproject.toml` build requires and `setup.py` install_requires against the environment's `numpy==2.0.2`, so the conflict is structural rather than transient; the extension has zero tracked BSM3 imports; and official LFS 1.0.0 release notes state it was "completely eliminated". The documented conda sequence installs cleanly end to end (Python 3.12.14, `bsm3-0.1.4`). Reproduced: 12 documentation tests, 13 passed / 5 deselected fast E175, strict Sphinx build, Ruff groups at 53/4/18/15/17 paths matching the report, clean clone with empty status, and 8/8 + 394/394 preservation. | Fresh Python 3.12 docs environment builds HTML with warnings as errors; **the documented core install executes successfully from an empty Python 3.12 environment**; CI carries the docs gate; no template prose, broken toctree target, unsupported guarantee, or incomplete public export inventory remains; docs do not require DAFoam, VortexAD, MPI, or local untracked assets to build. |
| M2.2 | Final documentation accuracy and clean-clone acceptance. | Claude reviews | **ACCEPTED — Turn 64; M2 CLOSED.** Documentation accuracy verified against source, not against the handoff: `print_summary` prose matches its body and no longer claims load stepping; PyVista's eager import is real (LFS `__init__` imports `core.function_set`, which imports PyVista at module level); recorder ownership is scoped to public `GeometryModel`/`mm.run`; the README labels its snippet a call shape and points at the runnable script; and `tests/test_documentation.py` is an `ast`-based export-inventory guard with exact set equality, described as nothing more. Two minor residues were found and deferred to the next implementation turn rather than held against M2: the `print_summary` **docstring** in production source still claims load stepping, and the documented install assumes a conda environment that ships `setuptools` (a bare `python -m venv` fails step 3 under `--no-build-isolation`). | Every command/path/public name is checked against the implementation; clean clone builds docs and runs the documentation tests; no unsupported integration or hosting claim. |
...
the VortexAD adapter, mesh generation, or M3 repository pruning.
...
| ID | Task | Owner | Status | Acceptance |
|----|------|-------|--------|------------|
| M4.1 | Make the E175 examples honest and usable: global vertex classifications, projection convergence, exact seam retention, corrected free-region terminology, explicit FD workflow, a clean quad-panel example, and measured n-gon calibration evidence. | Codex implements; Claude reviews | **ACCEPTED — Turn 68.** Claude re-derived the correction independently: `_global_intersection_vertices` was unit-checked on both branches, including mirror-sign application, reordering of unsorted retained-half IDs, exact symmetry-plane handling (`mirror_sign[plane, axis] == 0.0`, matching `reconstruct_full_from_half`), and both misalignment guards. The before/after was demonstrated in a disposable clone by reverting only the pipeline hunk: **1 failed, `assert (97, 3) == (194, 3)`** at `tests/test_e175_example.py:477`, then **1 passed in 84.46 s** on the corrected tree. Since the accepted `d0d1ff0`, only the pipeline helper, the API page, and one test changed; `load_stepping.py`, `projection.py`, `mesh_motion_config.py`, `mesh_motion.py`, and `geometry_model.py` are untouched. All gates reproduced. | Exact intersection rows bypass closest-point projection while retaining the intersection VJP; public full-mesh classifications and projection status agree with the diagnostic dump; `free_span_fraction` is bounded in `(0, 1]`; the basic triangle example runs at full scale; the clean panel asset replaces the 114-inversion baseline; docs/tests/derivative gates stay green; n-gon weights are measured rather than guessed. |
| M4.1a | Lock the GAMMA public identity and plan a clean-break migration before the first public documentation release. Distinguish the display name, repository slug, Python distribution, import namespace, and RTD slug rather than assuming they must all be bare `gamma`. | Claude plans; user rules; Codex implements; Claude reviews | **ACCEPTED — Turn 72.** Claude reproduced every gate and built the artifact independently: wheel `gamma_mdo-0.2.0a1-py3-none-any.whl` from a clean clone of `d04dc8f`, `pip show gamma-mdo` reporting `0.2.0a1`, and an out-of-tree `import bsm3` from `/tmp` resolving to **site-packages**, not the checkout. Scope was exactly 16 paths; `bsm3/__init__.py` changed only its version line; `tests/test_documentation.py` changed only the four authorized locations. Commit `d04dc8f` uses the user-corrected expansion *Geometry-Aware Mesh Movement Analysis*, which supersedes the longer Turn-70 planning expansion. Repository `GAMMA-MDO`, distribution `gamma-mdo`, RTD slug `gamma-mdo`, and version `0.2.0a1` are prepared. **The import namespace deliberately stays `bsm3`.** Wheel metadata and an out-of-tree import on the validated stack agree; no namespace directory moved. PyPI/RTD 404 results remain availability observations, not reservations. | The expansion and every technical identifier are explicit; bare `gamma` is rejected as an unavailable/overloaded distribution name; version policy is reconciled with the current `0.1.4`; the dirty tree has a concrete preservation strategy; implementation and clean-clone gates pass. |
| M4.1b | Publish an initial versioned GAMMA alpha on Read the Docs after the identity migration. | Claude plans; Codex prepares; user authorizes external publication | **ACCEPTED — Turn 72; external actions not started.** `.readthedocs.yaml` now requests HTML only (`formats: []`) and retains the hermetic docs-only install. Strict Sphinx 9.1.0 passes in place and from a clean clone. Repository rename, RTD project creation/webhook, tag creation/push, and any PyPI reservation/publication remain explicit user actions. | Strict HTML documentation builds under the final public identity from pinned docs-only dependencies; the RTD project and tag policy are explicit; no repository rename, tag, push, PyPI reservation, or RTD mutation occurs without user authorization. |
| M4.2 | Add a tracked VortexAD adapter and fuel-burn optimization example against pinned external revisions, using `mm.run` rather than resurrecting removed APIs. | Codex implements; Claude reviews | **ACCEPTED — Turn 75; M4 CLOSED.** Claude verified the pin against the official repository without installing it: commit `8c5bc86` exists in `LSDOlab/VortexAD` and its `__init__.py` exports all three names; all 11 `solver_input_dict` keys exist in `default_input_dict`; `CL`/`CDi`/`L`/`Di` are all in `output_options_dict` at shape `(num_nodes,)`; `solver_mode` defaults to `steady` and `wake_mode` to `fixed`, so `reuse_AIC=True` applies; `find_cell_adjacency` returns the 5-tuple indexed 0/1/3/4 as used; `TE_detection`'s keywords and 4-tuple return match. Breguet re-derived and dimensionally checked. The composed fake-solver test carries a mesh amplitude through CL/CDi, total drag and `compute_fuel_burn` to the fuel scalar and matches centered FD. All gates reproduced. | A supported panel solve and optimization run from tracked code; analytic derivatives are checked through the geometry-to-aerodynamics objective; DAFoam remains optional. |
...
| ID | Task | Owner | Status | Acceptance |
|----|------|-------|--------|------------|
| M7.1 | Restore literal CI Ruff coverage for the two production modules added by M4.2. | Codex implements; Claude reviews | **ACCEPTED — Turn 77; M7 CLOSED.** Four added lines, two per step, nothing else: path counts moved 53→55 and 18→20 with the other three steps unchanged at 4/15/17, no pre-existing path lost, every non-Ruff step byte-identical, and both modules occurring exactly twice in the whole file. YAML parses with the same 11 ordered step names as `fb833d5`. All five Ruff blocks were extracted from the edited file and executed verbatim: clean. Part-B ruling upheld against the workflow's own pre-existing comment, which Codex did not touch. Commit `b5e14f6` adds only `fuel_burn.py` and `panel_aerodynamics.py` to the critical-static and surface-core docstring steps. The five literal commands pass at path counts 55/4/20/15/17. The example and two tests remain outside the deliberately selective lint lists: the full test job executes both tests, the example has a structural AST test, and this correction does not redefine repository-wide lint policy or add a workflow step. | Both production modules appear in exactly the intended two Ruff steps; workflow shape is unchanged; all five commands, tests, docs, and preservation gates pass. |
