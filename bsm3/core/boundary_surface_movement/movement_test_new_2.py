...
def _mean_incident_edge_lengths(points: np.ndarray, adjacency: list[np.ndarray]) -> np.ndarray:
    ...
    for vertex_index, neighbors in enumerate(adjacency):
        if neighbors.size == 0:
            continue
        mean_edge_lengths[vertex_index] = np.linalg.norm(
            points[neighbors] - points[vertex_index],
            axis=1,
        ).mean()
    fallback = mean_edge_lengths[mean_edge_lengths > 0.0].mean() if np.any(mean_edge_lengths > 0.0) else 1.0
    mean_edge_lengths[mean_edge_lengths <= 0.0] = fallback
    return mean_edge_lengths


def _compute_vertex_normals(points: np.ndarray, triangles: np.ndarray, *, eps: float = 1e-14) -> np.ndarray:
    points = np.asarray(points, dtype=float)
    triangles = np.asarray(triangles, dtype=int)
    face_normals = _triangle_unit_normals(points, triangles, eps=eps)
    vertex_normals = np.zeros_like(points)
    for local_vertex_index in range(3):
        np.add.at(vertex_normals, triangles[:, local_vertex_index], face_normals)

    norms = np.linalg.norm(vertex_normals, axis=1)
    valid = norms > eps
    vertex_normals[valid] /= norms[valid, None]
    return vertex_normals


def _mesh_curvature_signal_from_normals(
    points: np.ndarray,
    triangles: np.ndarray,
    ...
    *,
    ...
    points = np.asarray(points, dtype=float)
    normals = _compute_vertex_normals(points, triangles, eps=eps)
    mean_edge_lengths = _mean_incident_edge_lengths(points, adjacency)
    curvature_signal = np.zeros(points.shape[0], dtype=float)

    for vertex_index, neighbors in enumerate(adjacency):
        if neighbors.size == 0:
            continue
        normal_differences = np.linalg.norm(
            normals[neighbors] - normals[vertex_index],
            axis=1,
        )
        curvature_signal[vertex_index] = normal_differences.mean() / max(mean_edge_lengths[vertex_index], eps)

    curvature_signal[~np.isfinite(curvature_signal)] = 0.0
    return curvature_signal, normals, mean_edge_lengths


def _apply_mesh_based_curvature_attraction_step(
    points: np.ndarray,
    *,
    triangles: np.ndarray,
    adjacency: list[np.ndarray],
    ...
    seam_support: np.ndarray,
    curvature_percentile: float,
    seam_bias: float,
    support_radius_factor: float,
    max_attractors_per_vertex: int,
    step_fraction: float,
    num_iterations: int,
    eps: float = 1e-14,
) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    points = np.asarray(points, dtype=float).copy()
    variable_indices = np.asarray(variable_indices, dtype=int).reshape((-1,))
    if variable_indices.size == 0 or int(num_iterations) <= 0 or float(step_fraction) <= 0.0:
        return (
            points,
            np.zeros(points.shape[0], dtype=float),
            np.zeros(points.shape[0], dtype=float),
            np.zeros_like(points),
        )

    seam_support_full = None if seam_support is None else np.asarray(seam_support, dtype=float).reshape((-1,))
    protected_full = None if protected_mask is None else np.asarray(protected_mask, dtype=bool).reshape((-1,))
    final_curvature_signal = np.zeros(points.shape[0], dtype=float)
    final_influence = np.zeros(points.shape[0], dtype=float)
    total_displacement = np.zeros_like(points)

    for _ in range(int(num_iterations)):
        curvature_signal, normals, mean_edge_lengths = _mesh_curvature_signal_from_normals(
            points,
            triangles,
            adjacency,
            eps=eps,
        )
        variable_points = points[variable_indices]
        variable_normals = normals[variable_indices]
        variable_mean_edge_lengths = mean_edge_lengths[variable_indices]
        variable_curvature = curvature_signal[variable_indices]
        variable_protected = (
            np.zeros(variable_indices.size, dtype=bool)
            if protected_full is None
            else protected_full[variable_indices]
        )

        threshold = np.percentile(variable_curvature, curvature_percentile)
        candidate_strengths = np.maximum(variable_curvature - threshold, 0.0)
        if np.max(candidate_strengths) <= eps:
            candidate_strengths[np.argmax(variable_curvature)] = max(np.max(variable_curvature), 1.0)
        candidate_strengths /= max(np.max(candidate_strengths), eps)

        if seam_support_full is not None:
            candidate_strengths *= 1.0 + float(seam_bias) * seam_support_full[variable_indices]

        candidate_local_indices = np.flatnonzero(candidate_strengths > 0.0)
        if candidate_local_indices.size == 0:
            candidate_local_indices = np.array([int(np.argmax(variable_curvature))], dtype=int)
            candidate_strengths[candidate_local_indices[0]] = 1.0

        candidate_points = variable_points[candidate_local_indices]
        candidate_strengths = candidate_strengths[candidate_local_indices]

        support_radius = max(float(support_radius_factor) * np.mean(variable_mean_edge_lengths), eps)
        decay_length = max(0.5 * support_radius, eps)
        max_attractors = max(1, int(max_attractors_per_vertex))

        directions = np.zeros_like(variable_points)
        influence = np.zeros(variable_indices.size, dtype=float)

        for local_index, point in enumerate(variable_points):
            if variable_protected[local_index]:
                continue
            vectors = candidate_points - point
            distances = np.linalg.norm(vectors, axis=1)
            valid = (distances > eps) & (distances <= support_radius)
            if not np.any(valid):
                continue

            valid_indices = np.flatnonzero(valid)
            if valid_indices.size > max_attractors:
                nearest_order = np.argsort(distances[valid_indices])[:max_attractors]
                valid_indices = valid_indices[nearest_order]

            valid_distances = distances[valid_indices]
            unit_vectors = vectors[valid_indices] / valid_distances[:, None]
            weights = np.exp(-valid_distances / decay_length) * candidate_strengths[valid_indices]
            weighted_direction = np.sum(weights[:, None] * unit_vectors, axis=0)

            normal = variable_normals[local_index]
            normal_norm = np.linalg.norm(normal)
            if normal_norm > eps:
                normal_unit = normal / normal_norm
                weighted_direction = weighted_direction - np.dot(weighted_direction, normal_unit) * normal_unit

            direction_norm = np.linalg.norm(weighted_direction)
            if direction_norm <= eps:
                continue

            directions[local_index] = weighted_direction / direction_norm
            influence[local_index] = np.clip(np.sum(weights), 0.0, 1.0e6)

        step_sizes = float(step_fraction) * variable_mean_edge_lengths * np.tanh(influence)
        step_sizes[variable_protected] = 0.0
        displacements = step_sizes[:, None] * directions
        points[variable_indices] = points[variable_indices] + displacements
        total_displacement[variable_indices] += displacements
        final_curvature_signal = curvature_signal
        final_influence[:] = 0.0
        final_influence[variable_indices] = influence

    return points, final_curvature_signal, final_influence, total_displacement
...
def _compute_vertex_ring_distances(
    neighbors: list[np.ndarray],
    source_indices: np.ndarray,
    *,
    ...
    source_indices = np.asarray(source_indices, dtype=int)
    ring_distances = np.full(len(neighbors), np.inf, dtype=float)
    queue: deque[int] = deque()

    for source_index in np.unique(source_indices):
        source_index = int(source_index)
        ring_distances[source_index] = 0.0
        queue.append(source_index)

    while queue:
        vertex_index = queue.popleft()
        current_distance = ring_distances[vertex_index]
        if max_rings is not None and current_distance >= float(max_rings):
            continue

        for neighbor_index in neighbors[vertex_index]:
            neighbor_index = int(neighbor_index)
            candidate_distance = current_distance + 1.0
            if candidate_distance < ring_distances[neighbor_index]:
                ring_distances[neighbor_index] = candidate_distance
                queue.append(neighbor_index)

    return ring_distances


def _feature_buffer_scale_from_mask(
    neighbors: list[np.ndarray],
    source_mask: np.ndarray,
    buffer_rings: int,
) -> np.ndarray:
    scale = np.ones((len(neighbors),), dtype=float)
    if buffer_rings <= 0:
        return scale

    source_indices = np.where(np.asarray(source_mask, dtype=bool))[0]
    if source_indices.size == 0:
        return scale

    ring_distances = _compute_vertex_ring_distances(
        neighbors,
        source_indices,
        max_rings=buffer_rings,
    )
    within_buffer_mask = (
        np.isfinite(ring_distances)
        & (ring_distances <= float(buffer_rings))
    )
    if np.any(within_buffer_mask):
        normalized_ring_distances = (
            ring_distances[within_buffer_mask] / max(float(buffer_rings), 1.0)
        )
        scale[within_buffer_mask] = _quintic_smoothstep(normalized_ring_distances)
...
if __name__ == "__main__":
    plot = True
    plot_step8_prediction = True
    debug_projections = False
    alpha = 1.0
    sigma_phi = 0.05
    participation_mode = "hybrid"
    participation_sharpening = 1.0
    rbf_fit_mode = "weighted_least_squares"
    rbf_kernel = "multiquadric"
    rbf_kernel_scale_factor = 0.1
    rbf_regularization_factor = 1e-6
    rbf_polynomial_regularization_factor = 1e-6
    ...
    wing_root_rotation_degrees = 5.0
    ...
    seam_support_sigma_factor = 0.08
    seam_training_support_cutoff = 0.05
    step9_projection_mode = "smooth_union_ks"
    step9_smooth_union_ks_rho_start = 12.5
    step9_smooth_union_ks_rho_end = 12.5
    step9_smooth_union_num_iterations = 5
    step9_smooth_union_tolerance = 1e-10
    step9_curvature_attraction_num_iterations = 5
    step9_curvature_attraction_percentile = 85.0
    step9_curvature_attraction_step_fraction = 0.08
    step9_curvature_attraction_support_radius_factor = 12.0
    step9_curvature_attraction_max_attractors_per_vertex = 16
    step9_curvature_attraction_seam_bias = 0.5
    step9_pre_projection_laplacian_step = 0.25
    sharp_feature_protection = True
    sharp_feature_angle_degrees = 35.0
    sharp_feature_include_boundary = True
    step8_sharp_feature_buffer_rings = 3
    max_local_training_points = 900
    max_wing_anchor_training_points = 200
    max_fuse_anchor_training_points = 900
    ...
    nose_mask = vertices[:, 0] <= x_min + 0.1 * dx
    ...
    step8_sharp_feature_buffer_scale = np.ones(vertices.shape[0], dtype=float)
    if sharp_feature_protection:
        step8_sharp_feature_vertex_mask = _sharp_feature_vertex_mask(
            vertices,
            triangles,
            feature_angle_degrees=sharp_feature_angle_degrees,
            include_boundary=sharp_feature_include_boundary,
        )
        step8_sharp_feature_buffer_scale = _feature_buffer_scale_from_mask(
            vertex_neighbors,
            step8_sharp_feature_vertex_mask,
            step8_sharp_feature_buffer_rings,
        )
    ...
    step8_sharp_feature_variable_scale = step8_sharp_feature_buffer_scale[variable_indices]
    ...
    num_additional_interp_vertices = 1000
    ...
    step8_sharp_feature_interpolation_scale = step8_sharp_feature_buffer_scale[
    ...
    local_training_candidate_mask = seam_interpolation_mask | (
        ...
        & (step8_sharp_feature_interpolation_scale > 1e-12)
    )
    ...
    if interpolation_local_training_points.shape[0] > 0:
        interpolation_local_training_targets = interpolation_local_residuals[
            local_training_indices
        ].copy()
        local_training_seam_mask = seam_interpolation_mask[local_training_indices]
        local_training_feature_scale = step8_sharp_feature_interpolation_scale[
            local_training_indices
        ]
        if np.any(~local_training_seam_mask):
            interpolation_local_training_targets[~local_training_seam_mask] *= (
                local_training_feature_scale[~local_training_seam_mask, None]
            )
        interpolation_local_training_raw_residuals = (
            interpolation_local_training_targets
        ...
        if np.any(~local_training_seam_mask):
            interpolation_local_training_sample_weights[~local_training_seam_mask] *= (
                local_training_feature_scale[~local_training_seam_mask]
            )
        interpolation_local_training_sample_weights[
    ...
    variable_local_support *= step8_sharp_feature_variable_scale
    ...
    step9_curvature_signal = np.zeros(vertices.shape[0], dtype=float)
    step9_curvature_influence = np.zeros(vertices.shape[0], dtype=float)
    ...
    step9_curvature_attracted_variable_vertices = predicted_variable_vertices.copy()
    if (step9_curvature_attraction_num_iterations > 0 or step9_pre_projection_laplacian_step > 0.0) and variable_indices.size > 0:
    ...
    if step9_curvature_attraction_num_iterations > 0 and variable_indices.size > 0:
        step9_seam_support = np.zeros(vertices.shape[0], dtype=float)
        step9_seam_support[variable_indices] = variable_local_support

        (
            step9_curvature_attracted_vertices,
            step9_curvature_signal,
            step9_curvature_influence,
            _step9_curvature_displacement,
        ) = _apply_mesh_based_curvature_attraction_step(
            step9_predicted_vertices,
            triangles=triangles,
            adjacency=vertex_neighbors,
            variable_indices=variable_indices,
            protected_mask=step9_sharp_feature_vertex_mask,
            seam_support=step9_seam_support,
            curvature_percentile=step9_curvature_attraction_percentile,
            seam_bias=step9_curvature_attraction_seam_bias,
            support_radius_factor=step9_curvature_attraction_support_radius_factor,
            max_attractors_per_vertex=step9_curvature_attraction_max_attractors_per_vertex,
            step_fraction=step9_curvature_attraction_step_fraction,
            num_iterations=step9_curvature_attraction_num_iterations,
        )
        step9_curvature_attracted_variable_vertices = step9_curvature_attracted_vertices[variable_indices]

    step9_smoothed_variable_vertices = step9_curvature_attracted_variable_vertices.copy()
    if step9_pre_projection_laplacian_step > 0.0 and variable_indices.size > 0:
        step9_predicted_vertices = vertices.copy()
        step9_predicted_vertices[variable_indices] = step9_curvature_attracted_variable_vertices
        if wing_anchor_indices.size > 0:
            step9_predicted_vertices[wing_anchor_indices] = moved_wing_anchor_vertices
        if fuse_anchor_indices.size > 0:
            step9_predicted_vertices[fuse_anchor_indices] = fuse_anchor_vertices
    ...
    pre_projection_curvature_attraction_correction = (
        step9_curvature_attracted_variable_vertices - predicted_variable_vertices
    )
    pre_projection_smoothing_correction = (
        step9_smoothed_variable_vertices - step9_curvature_attracted_variable_vertices
    )
    ...
    print("    Step 8 sharp-feature buffer rings:", int(step8_sharp_feature_buffer_rings))
    print(
        "    Step 9 curvature-attraction iterations:",
        int(step9_curvature_attraction_num_iterations),
    )
    print(
        "    Step 9 curvature-attraction percentile:",
        f"{step9_curvature_attraction_percentile:.6e}",
    )
    print(
        "    Step 9 curvature-attraction step fraction:",
        f"{step9_curvature_attraction_step_fraction:.6e}",
    )
    print(
        "    Step 9 curvature-attraction support radius factor:",
        f"{step9_curvature_attraction_support_radius_factor:.6e}",
    )
    print(
        "    Step 9 curvature-attraction seam bias:",
        f"{step9_curvature_attraction_seam_bias:.6e}",
    )
    print(
    ...
    print(
        "    Step 9 curvature signal:",
        _format_stats(step9_curvature_signal[variable_indices]),
    )
    print(
        "    Step 9 curvature-attraction influence:",
        _format_stats(step9_curvature_influence[variable_indices]),
    )
    print(
        "    Step 9 curvature-attraction correction:",
        _format_stats(np.linalg.norm(pre_projection_curvature_attraction_correction, axis=1)),
    )
    print(
    ...
    if plot:
        ...
        flipped_triangle_plot = None
        if flipped_triangle_indices.size > 0:
            flipped_triangle_faces = np.empty(
                (flipped_triangle_indices.size, 4), dtype=np.int64
            )
            flipped_triangle_faces[:, 0] = 3
            flipped_triangle_faces[:, 1:] = triangles[flipped_triangle_indices]
            flipped_triangle_plot = pv.PolyData(
                updated_vertices,
                flipped_triangle_faces.ravel(),
            )
        ...
        if flipped_triangle_plot is not None:
            plotter.add_mesh(
                flipped_triangle_plot,
                color="magenta",
                edge_color="yellow",
                opacity=1.0,
                show_edges=True,
